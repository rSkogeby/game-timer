...
const App: React.FC = () => {
  ...
  const handleToggle = (): void => {
    ...
    Animated.timing(iconMovement, { duration: 2000, easing: Easing.inOut(Easing.exp), toValue: toggle ? 0 : -130, useNativeDriver: true }).start()
  ...
  // useEffect(() => {
  //   Animated.timing(backgroundFade, { duration: 2000, toValue: 1, useNativeDriver: true }).start()

  //   Animated.timing(logoFade, { duration: 2000, toValue: 1, useNativeDriver: true }).start()
  //   Animated.timing(iconMovement, { duration: 2000, easing: Easing.inOut(Easing.exp), toValue: -50, useNativeDriver: true }).start()
  //   Animated.timing(textMovement, { duration: 2000, easing: Easing.inOut(Easing.exp), toValue: 50, useNativeDriver: true }).start()

  //   setTimeout(() => { Animated.timing(logoMovement, { duration: 2000, easing: Easing.inOut(Easing.exp), toValue: -350, useNativeDriver: true }).start() }, 2250)
  // }, [])

  if (!appIsReady) {
    return (
      <Animated.View style={{ backgroundColor: 'blue', flexGrow: 1 }}>
        <Animated.View
          style={{
            alignItems: 'center',
            flexGrow: 1,
